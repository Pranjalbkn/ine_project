...
export async function getStats() {
  const result = await pool.query(`
    select
      (select count(*)::integer from scrape_log) as "totalScrapes",
...
export async function isTracked(productId) {
  const result = await pool.query(
    'select 1 from tracked_products where product_id = $1',
    [productId],
  );
...
export async function addTracked(product) {
  const result = await pool.query(`
    insert into tracked_products
      (product_id, name, brand, category, sku)
    values ($1, $2, $3, $4, $5)
...
async function scrapeAndSave(productId, options = {}) {
  ...
  try {
    ...
    attemptLog = error.attempts?.length
      ? error.attempts
      : [
          {
            attempt: 1,
...
export function runTrackedScrape(productId, options = {}) {
  const headless = options.headless ?? true;

  // Separate concurrent requests by browser mode.
  const cacheKey = `${productId}:${headless}`;

  // Share one scrape when two requests ask for the same product
  // using the same browser mode.
  if (!activeTrackedScrapes.has(cacheKey)) {
    const task = scrapeAndSave(productId, {
      ...options,
      headless,
    }).finally(() => {
      activeTrackedScrapes.delete(cacheKey);
    });

    activeTrackedScrapes.set(cacheKey, task);
  }

  return activeTrackedScrapes.get(cacheKey);
}