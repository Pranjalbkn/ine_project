...
const localBrowserFolder = resolve(dirname(fileURLToPath(import.meta.url)), '../.playwright-browsers');
if (!process.env.PLAYWRIGHT_BROWSERS_PATH && existsSync(localBrowserFolder)) {
...
export function parseAmount(text) {
  ...
  numberText = numberText.replace(/[.,]00$/, '').replace(/[.,]/g, '');
...
async function dismissCookies(page) {
  const declineButton = page.getByRole('button', { name: 'Decline cookies' });
  // This overlay appears after a random delay and can demand several clicks.
  await declineButton.waitFor({ state: 'visible', timeout: 2_500 }).catch(() => {});
  for (let clickNumber = 0; clickNumber < 4; clickNumber += 1) {
...
async function revealPrice(page) {
  const priceCard = page.locator('.price-block').first();
  await priceCard.waitFor({ state: 'visible', timeout: 25_000 });
  ...
  if (!bounds) throw new Error('Price area has no visible bounding box');
  await page.mouse.move(bounds.x + 15, bounds.y + bounds.height / 2);
  for (let step = 1; step <= 12; step += 1) {
    const xPosition = bounds.x + 15 + ((bounds.width - 30) * step) / 13;
    const yPosition = bounds.y + bounds.height * (0.35 + (step % 3) * 0.12);
  ...
  await revealButton.waitFor({ state: 'visible', timeout: 5_000 });
  await dismissCookies(page);
  try {
    await revealButton.click({ timeout: 4_000 });
  } catch (error) {
...
async function extractReading(page, expectedName) {
  return page.locator('.price-block.price-success').evaluate((priceCard, expected) => {
    const priceArea = priceCard.querySelector('.price-main');
    const stockBadge = priceCard.querySelector('.stock-badge');
    ...
    const visiblePrices = [...priceArea.children].filter((element) => {
    ...
    return {
      ...
      outOfStock: stockBadge.classList.contains('out-stock'),
    };
  }, expectedName);
}
...
export function validateReading(rawReading) {
  ...
  let stock;
  if (rawReading.outOfStock && /^Out of stock$/i.test(rawReading.rawStock)) {
    stock = 0;
  } else {
    const stockCount = rawReading.rawStock.match(/\b(\d+)\b/);
    const stockText = rawReading.rawStock.toLowerCase();
    if (!stockCount || (!stockText.includes('stock') && !stockText.includes('left'))) {
      throw new Error(`Unrecognized stock text: ${rawReading.rawStock}`);
    }
    stock = Number(stockCount[1]);
    if (!Number.isSafeInteger(stock) || stock <= 0) {
      throw new Error(`Invalid stock count: ${rawReading.rawStock}`);
    }
  }
  return { name: rawReading.name, price, currency: 'INR', stock };
}
...
export async function scrapeProduct(productId, options = {}) {
  ...
  const maximumAttempts = options.attempts ?? DEFAULT_ATTEMPTS;
  const attemptLog = [];
  ...
  try {
    for (let attempt = 1; attempt <= maximumAttempts; attempt += 1) {
      ...
      try {
        ...
        await page.goto(`${STORE_ORIGIN}/product/${numericProductId}`, {
        ...
        return {
          ...
          productUrl: `${STORE_ORIGIN}/product/${numericProductId}`,
          ...reading,
        ...
        const attemptRecord = {
          ...
          outcome: attempt < maximumAttempts ? 'retried' : 'failed',
