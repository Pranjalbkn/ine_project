  ...
  return <section className="intro">
    ...
    <label className="search-box">
      ...
      <input
        ...
        autoComplete="off"
      />
      {query && <button type="button" onClick={() => onQueryChange('')} aria-label="Clear search">×</button>}
    </label>
    ...
    {trackedProducts.length > 0 && <div className="tracked-strip">
      <span>TRACKING</span>
      {trackedProducts.map((item) => <button type="button" key={item.productId} onClick={() => onSelectProduct(item.productId)}>{item.name}</button>)}
    </div>}
  </section>;
}
